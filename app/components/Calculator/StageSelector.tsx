    ...
    <div className="bg-black border border-white/10 p-6 hover:border-white/20 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-4">
        {data.stages.map((stage) => {
          const Icon = stageIcons[stage.id] ?? Rocket;
          ...
          return (
            ...
            >
              ...
              <div className={`font-semibold mb-1 ${isSelected ? 'text-white' : 'text-white/70'}`}>{t(stage.tKey)}</div>
              <div className="text-xs text-white/50">{t(stage.descKey)}</div>
            </button>
          );
        })}
      </div>
