...
const CATEGORY_ORDER = ['state', 'forms', 'ui', 'tooling'] as const;
...
export default function PackagesPage() {
  ...
  const cardOrder = grouped.flatMap((group) => group.items.map((pkg) => pkg.name));
          ...
          <div className="flex flex-col gap-14">
            {grouped.map((group) => (
              <section key={group.category}>
                <h2 className="text-white/40 text-xs uppercase tracking-widest mb-5">
                  {t(`packages.categories.${group.category}`)}
                </h2>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {group.items.map((pkg) => (
                    <motion.article
                      ...
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.4, delay: cardOrder.indexOf(pkg.name) * 0.04 }}
                    >
